...
@st.cache
def scale_data(df, target, scal_opt):
...
scale_bt = st.button('Scale data')

scaled_df = scale_data(df, target_col, scal_opt)
...
with pca_header:
    ...
    def do_pca(data, num_pca, target_col):
        pca = PCA(n_components= num_pca)
  
        pca_fit = pca.fit_transform(data.select_dtypes(include='number'))

        pca_col = ["PC"+str(i) for i in list(range(1, num_pca+1))]

        pca_plot = pd.DataFrame(pca_fit, columns = pca_col[:num_pca])
        ...
        return pca_plot, features, loadings, loadings_df, fig

 
    pca_plot, features, loadings, loadings_df, fig = do_pca(scaled_df, num_pca, target_col)
    ...
    @st.cache
    def do_kpca(data, num_pca, kpca_kernel, gamma_kpca, target_col):
        kpca = KernelPCA(n_components = num_pca, kernel = kpca_kernel, gamma = gamma_kpca)
        kpca_res = kpca.fit_transform(data.select_dtypes(include='number'))

        kpca_col = ["PC"+str(i) for i in list(range(1, num_pca+1))]

        kpca_plot = pd.DataFrame(kpca_res, columns = kpca_col[:num_pca])
        ...
        return kpca_plot, kpca_fig

    kpca_plot, kpca_fig = do_kpca(scaled_df, num_pca, kpca_kernel, gamma_kpca, target_col)
    ...
    @st.cache
    def do_tsne(data, tsne_num_comp, tsne_perp, tsne_early_ex, tsne_rate, tsne_num_iter, target_col):
        ...
        tsne_res = tsne.fit_transform(data.select_dtypes(include='number'))
        ...
        return tsne_df, tsne_fig

    tsne_df, tsne_fig = do_tsne(scaled_df, tsne_num_comp, tsne_perp, tsne_early_ex, tsne_rate, tsne_num_iter, target_col)
    st.write(tsne_fig)
...
with classification:
    ...
    if task_info == 'Classification':
        ...
        target_var = scaled_df[target_sel]
    ...
    @st.cache
    def make_class_df(df, target_var):
        class_df = pd.DataFrame(df.select_dtypes(include = 'number'), columns = df.select_dtypes(include='number').columns)
        if task_info == 'Classification':
            class_df['target'] = target_var
        return class_df
    
    class_df = make_class_df(scaled_df, target_var)
    ...
    if task_info == 'Classification':
        ...
        x = class_df.drop([target_sel], axis = 1)
        y = class_df[target_sel]
    ...
    @st.cache
    def probs_df(probs, l_enc):
        probs = pd.DataFrame(probs, columns = l_enc.inverse_transform(class_df['target'].unique()))
        return probs
    if task_info == 'Classification':
        probs = probs_df(probs, l_enc)

    if task_info == 'Classification' and st.checkbox('Show predicted probabilities of assigned class:'):
        st.dataframe(probs)
    ...
    if task_info == 'Classification' and st.checkbox("Show decision boundary plot"):
...
with clustering:
    ...
    @st.cache
    def do_kmeans(df, num_clusters, num_init):
        kmeans = KMeans(n_clusters = num_clusters, n_init = num_init)
        kmeans.fit(df.select_dtypes(include='number'))
        pred_clusters = kmeans.predict(df.select_dtypes(include='number'))
        kmeans_labels = kmeans.labels_
        cluster_centers = kmeans.cluster_centers_
        return pred_clusters, kmeans_labels, cluster_centers

    pred_clusters, kmeans_labels, cluster_centers = do_kmeans(scaled_df, num_clusters, num_init)
    ...
    @st.cache
    def do_sp_clust(df, sp_num_clusters, sp_num_init, sp_gamma, sp_affinity, num_neigh):
        ...
        sp_clusters = sp_model.fit_predict(df.select_dtypes(include='number'))
        sp_labels = sp_model.labels_
        return sp_clusters, sp_labels
    
    sp_clusters, sp_labels = do_sp_clust(scaled_df, sp_num_clusters, sp_num_init, sp_gamma, sp_affinity, num_neigh)
